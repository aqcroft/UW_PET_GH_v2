(function(){
  const style=document.createElement('style');
  style.textContent=`
    .splash-card.splash-v8 .splash-body{padding:1rem 1.05rem 1.05rem}
    .splash-card.splash-v8 .splash-body>p{margin:.65rem 0}
    .splash-card.splash-v8 .income-chip-grid{margin:.7rem 0;gap:.5rem}
    .splash-card.splash-v8 .income-chip{
      display:grid;
      grid-template-columns:minmax(0,1fr) 1.25rem auto;
      align-items:center;
      gap:.4rem;
      text-align:left;
      padding:.62rem .7rem;
    }
    .splash-card.splash-v8 .bonus-name{font-size:.92rem;font-weight:950;text-align:left;min-width:0}
    .splash-card.splash-v8 .bonus-gap{display:block}
    .splash-card.splash-v8 .bonus-meta{font-size:.86rem;font-weight:950;text-align:right;color:var(--blue);white-space:nowrap}
    .splash-card.splash-v8 .splash-mantra-button{width:100%;margin-top:.72rem;border:none;border-radius:.7rem;padding:.82rem 1rem;background:linear-gradient(135deg,var(--blue),var(--green));color:#fff;font-weight:950;font-size:.96rem;cursor:pointer;box-shadow:0 8px 22px rgba(29,79,145,.22)}
  `;
  document.head.appendChild(style);

  renderWelcome=function(){
    document.body.classList.remove('explorer-independent');
    document.getElementById('app').innerHTML=`${contactMarkup()}<section class="splash-card splash-v8" style="margin-top:10vh"><div class="splash-top"><h1>What happens when you support a new Partner?</h1></div><div class="splash-body"><p>Let's look at the three mentor Bonuses:</p><div class="income-chip-grid"><div class="income-chip"><span class="bonus-name">🤝Supporting bonuses</span><span class="bonus-gap"></span><span class="bonus-meta">Days 1-30</span></div><div class="income-chip"><span class="bonus-name">🎓30 Day Fast Start Mentor</span><span class="bonus-gap"></span><span class="bonus-meta">£400</span></div><div class="income-chip"><span class="bonus-name">🎓⚡Mentor Momentum</span><span class="bonus-gap"></span><span class="bonus-meta">Days 31-60</span></div></div><button class="splash-mantra-button" onclick="start()">Support - Momentum - Success🏆</button></div></section>${howPaidMarkup()}`;
  };

  function markV8(){
    const top=document.querySelector('.topbar .eyebrow span');
    if(top&&top.textContent!=='v8')top.textContent='v8';
  }
  const app=document.getElementById('app');
  if(app)new MutationObserver(markV8).observe(app,{childList:true,subtree:true});
  markV8();
  if(!state.started)render();
})();
