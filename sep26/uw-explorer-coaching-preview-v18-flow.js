(function(){
  if(document.documentElement.dataset.coachingPreviewV18Flow==='1')return;
  document.documentElement.dataset.coachingPreviewV18Flow='1';
  ...
  style.textContent=`
    .v9-coach.v18-hab-again{padding-bottom:1.25rem!important}
    .v18-momentum-transition{transition:opacity .45s ease,transform .45s ease!important}
    .v18-momentum-transition .v18-icon{display:block;font-size:2rem;line-height:1;margin-bottom:.55rem;text-align:center}
    .v18-momentum-transition .v18-line{display:block;margin-top:.42rem;line-height:1.38}
    .v18-momentum-transition .v18-pointer{display:block;font-size:1.85rem;line-height:1;margin-top:.55rem}
    .v18-hab-result .v18-hab-amount{display:block;margin-top:.72rem;font-size:1rem;font-weight:600;line-height:1.35}
  `;
  ...
  render=function(){existingRender();refineAfterRender();};

  function positionTransition(coach){
    const btn=document.querySelector('.add-customer-main');
    const rect=btn?.getBoundingClientRect();
    const height=coach.offsetHeight||110;
  ...
  function showMomentumTransition(){
    if(momentumTransitionComplete||document.querySelector('.v18-momentum-transition'))return;
    const coach=document.createElement('div');
    coach.className='v9-coach v9-coach-green v18-momentum-transition';
    coach.setAttribute('role','status');
    coach.setAttribute('aria-live','polite');
    coach.innerHTML='<span class="v18-icon">🚀</span><strong>You’re really flying!</strong><span class="v18-line"><strong>Customer Bonus + Momentum</strong><br>as we add a few customers</span><span class="v18-pointer">👇</span>';
    document.body.appendChild(coach);
    positionTransition(coach);
    requestAnimationFrame(()=>coach.classList.add('show'));

    baseSetTimeout(()=>coach.classList.remove('show'),4800);
    baseSetTimeout(()=>{
      coach.remove();
    ...
    for(const record of records){
      for(const node of record.addedNodes){
        ...
        if(text.includes('Momentum Bonus unlocked!')||text.includes('A new bonus is now in play')){
          if(node.dataset.v18MomentumReveal!=='1'){
            node.dataset.v18MomentumReveal='1';
        ...
        if(text.includes('High Activity Bonus unlocks again')||text.includes('High Activity Bonus kicks in again')){
          node.classList.add('v18-hab-again');
        }

        if(text.includes('High Activity Bonus Applied')&&text.includes('£1,000')&&text.includes('£1,400')){
          node.classList.add('v18-hab-result');
          node.innerHTML='<span class="v16-icon">🔥</span><strong>High Activity Bonus Applied</strong><span class="v18-hab-amount">£1,000 ➡️ £1,400</span>';
        }
      }
    }
  });
