(function(){
  if(document.documentElement.dataset.coachingPreviewV9==='1')return;
  document.documentElement.dataset.coachingPreviewV9='1';
  ...
  style.textContent=`
    ...
    @keyframes v9ResultPulse{
      0%,100%{transform:scale(1)}
      45%{transform:scale(1.075)}
    }
    @keyframes v9StepPulse{
    ...
    @keyframes v9ModalBackdropIn{from{opacity:0}to{opacity:1}}
    @keyframes v9ModalIn{
      from{opacity:0;transform:translateY(12px) scale(.985)}
      to{opacity:1;transform:translateY(0) scale(1)}
    }

    .v9-action-pulse{animation:v9ActionPulse 1.05s ease-in-out 1 both!important;position:relative;z-index:3}
    .v9-result-pulse{animation:v9ResultPulse .95s ease-in-out 1 both!important;position:relative;z-index:3}
    .v9-section-pulse{animation:v9SectionPulse 1.45s ease-in-out 1 both!important;position:relative;z-index:2}
    .bonus-faststart.v9-fast-stage{animation:v9FastStage 1.25s ease-in-out 1 both!important;background:#dbeaff!important;border-color:rgba(29,79,145,.58)!important;opacity:1!important}

    .v9-step-current{opacity:.42!important;filter:none!important}
    .v9-step-current.v9-step-pulse{animation:v9StepPulse 1.25s ease-in-out 1 both!important;transform-origin:center!important}
    .v9-step-locked{opacity:.18!important;filter:saturate(.28);pointer-events:none!important}
    .v9-step-done,.step-button.done.v9-step-done{opacity:1!important;filter:none!important;background:linear-gradient(145deg,#173c78,var(--blue))!important;border-color:var(--blue)!important;color:#fff!important}
    ...
    .v9-gated-add{pointer-events:none!important}
    .v9-gated-add.v9-gated-dim{opacity:.52!important}
    ...
    .v9-coach.v9-coach-green{background:#edf8f1;border-color:rgba(20,122,74,.28);color:#173d2b;box-shadow:0 14px 36px rgba(20,80,48,.16)}
    .v9-coach.show{opacity:1;transform:translate(-50%,-.16rem)}
    .v9-coach strong{font-weight:950}
    .v9-coach .coach-line{display:block;margin-top:.22rem;font-size:.95rem}
    .v9-coach .coach-money{display:block;margin-top:.32rem;font-size:1.02rem;font-weight:950}
    .v9-coach .coach-money.v9-normal-money{font-weight:600!important}
    ...
    .v9-unlock-copy{margin:.7rem 0 1rem;font-size:.98rem;line-height:1.42;text-align:center}
    .v9-unlock-copy strong{display:block;margin-bottom:.25rem}
    .v9-unlock-copy .unlock-line{display:block;font-weight:900}

    .notice-backdrop.v9-modal-enter{animation:v9ModalBackdropIn .48s ease both}
    .notice-backdrop.v9-modal-enter>.notice-popup{animation:v9ModalIn .48s cubic-bezier(.22,.8,.32,1) both}
    .notice-backdrop.v9-modal-exit{opacity:0!important;pointer-events:none!important;transition:opacity .5s ease!important}
    .notice-backdrop.v9-modal-exit>.notice-popup{opacity:0!important;transform:translateY(10px) scale(.985)!important;transition:opacity .5s ease,transform .5s cubic-bezier(.4,0,.2,1)!important}
    ...
    .v9-back-step:disabled{opacity:.32;cursor:not-allowed}
    ...
    @media(max-width:520px){
      ...
      .v9-back-step{width:4.2rem;padding:0 .35rem;font-size:.78rem}
    }
    @media(prefers-reduced-motion:reduce){
      .v9-action-pulse,.v9-result-pulse,.v9-section-pulse,.v9-step-pulse,.v9-fast-stage,
      .customer-row-hab-pulse,.bonus-hab.pulse,.customer-row-momentum-pulse,.bonus-momentum.pulse,
      .notice-backdrop.v9-modal-enter,.notice-backdrop.v9-modal-enter>.notice-popup{animation:none!important}
      .notice-backdrop.v9-modal-exit,.notice-backdrop.v9-modal-exit>.notice-popup{transition:none!important}
    }
  `;
  ...
  function busy(){return !!(state.habRevealInProgress||state.momentumRevealInProgress||state.fastStartRevealInProgress||ui.batchRunning||document.querySelector('.v9-coach')||document.querySelector('.notice-backdrop:not(.howpaid-overlay)'));}
  ...
  function pulse(el,type='action',delay=0){
    ...
    const cls=type==='result'?'v9-result-pulse':'v9-action-pulse';
    later(()=>{
  ...
  function pulseStep(el,delay=0){
    ...
    later(()=>{
      if(!el.isConnected)return;
      el.classList.remove('v9-step-pulse');void el.offsetWidth;el.classList.add('v9-step-pulse');
      later(()=>el.classList.remove('v9-step-pulse'),1300);
    },delay);
  }
  function pulseSection(){
    const target=columns()[1];
    if(!target||reducedMotion)return;
    target.classList.remove('v9-section-pulse');void target.offsetWidth;target.classList.add('v9-section-pulse');
    later(()=>target.classList.remove('v9-section-pulse'),1600);
  }
  ...
  function scheduleCoach(html,{delay=COACH_PRE_DELAY,after=null,dim=true,green=null}={}){
    setGate(true,dim);
    later(()=>{
      document.querySelector('.v9-coach')?.remove();
      const el=document.createElement('div');
      const phase2=green===null?(state.nextIntroDismissed||totalCount()>=6):!!green;
      el.className=`v9-coach${phase2?' v9-coach-green':''}`;
  ...
  window.addEventListener('resize',()=>document.querySelectorAll('.v9-coach').forEach(positionCoach));
  ...
  function enhanceActions(){
    const btn=addButton();
    if(btn){
      btn.classList.toggle('v9-gated-add',ui.gate);
      btn.classList.toggle('v9-gated-dim',ui.gate&&ui.gateDim);
      ...
      if(totalCount()===6&&ui.batchReady&&!ui.batchRunning){
    ...
    if(row&&!row.querySelector('.v9-back-step')){
      const back=document.createElement('button');
      back.type='button';
      back.className='v9-back-step';
      back.textContent='↩ Back';
      back.onclick=undo;
      row.prepend(back);
    }
    const back=row?.querySelector('.v9-back-step');
  ...
  function animateModalIn(backdrop,popup){
    ...
    ui.modalKey=key;
    backdrop.classList.add('v9-modal-enter');
    later(()=>backdrop.classList.remove('v9-modal-enter'),650);
  }
  ...
  function rewriteFastStartModal(popup){
    ...
    if(!state.fastStartRevealComplete){
      ...
      steps.forEach(step=>step.classList.remove('v9-step-current','v9-step-locked','v9-step-done'));
      if(steps[0]){
        if(state.customerPartnerUpgraded){steps[0].classList.add('v9-step-done','done');steps[0].style.opacity='1';}
        else steps[0].classList.add('v9-step-current');
      }
      if(steps[1]){
        if(state.ownAccountLinked){steps[1].classList.add('v9-step-done','done');steps[1].style.opacity='1';}
        else if(!state.customerPartnerUpgraded){steps[1].classList.add('v9-step-locked');steps[1].setAttribute('disabled','disabled');}
        else {steps[1].classList.add('v9-step-current');if(!state.explorerCriteriaBusy)steps[1].removeAttribute('disabled');}
      }
      ...
      if(!popup.querySelector('.v9-unlock-copy')){
  ...
  function rewriteFinalModal(popup){
    ...
    if(p)p.innerHTML='This is just an example.<br><br><strong>What would another £4,000-£5,000 mean to you over the next 60 days?</strong>';
  }
  ...
  function scheduleTransitionCoaches(before,after){
    if(after.count===1&&before.count===0&&!ui.firstCustomerCoachShown){
      ...
      scheduleCoach('<strong>First customer added</strong><span class="coach-line">3 service homeowner</span><span class="coach-money">£250 Customer Bonus</span>',{
        after:()=>later(()=>{if(totalCount()===1)pulse(addButton(),'action');},1200),green:false
      });
    }
    ...
    if(after.count===2&&before.count===1&&!ui.customer2CoachShown){
      ...
      setGate(true,true);
      pulse(columnTotals()[0],'result',120);
      scheduleCoach('<strong>Great stuff!</strong><span class="coach-line">Another £250 for your second customer</span>',{green:false});
    }

    if(after.count===3&&before.count===2&&!ui.customer3CoachShown){
      ui.customer3CoachShown=true;
      setGate(true,true);
      scheduleCoach('🔥 <strong>One more 3-service homeowner...</strong><span class="coach-line">your <strong>4th this month</strong> unlocks a new bonus 👇</span>',{green:false});
    }
    ...
    if(after.m1Hab&&!before.m1Hab&&!ui.postHabCoachShown){
      ...
      scheduleCoach('<strong>Yes, that\'s an extra £400!</strong><span class="coach-money v9-normal-money">£1,000 ➡️ £1,400</span>',{
        after:()=>later(()=>{if(totalCount()===4)pulse(addButton(),'action');},1200),green:false
      });
    }

    const momentumNow=new Set(state.momentumAppliedIds);
    const momentumBefore=new Set((before.momentumIds||'').split(',').filter(Boolean));
    if(momentumNow.has('c6')&&!momentumBefore.has('c6')&&!ui.customer6CoachShown){
      ui.customer6CoachShown=true;
      setGate(true,true);
      scheduleCoach('<strong>Your confidence is building!</strong><span class="coach-line">Let\'s add a few more Momentum customers</span>',{
        ...
        after:()=>{
          ...
          later(()=>{if(totalCount()===6)pulse(addButton(),'action');},1100);
        }
      });
    }
  ...
  render=function(){
    ...
    const momentumBefore=new Set((before.momentumIds||'').split(',').filter(Boolean));
    const momentumNow=new Set(state.momentumAppliedIds);
    if(momentumNow.has('c6')&&!momentumBefore.has('c6')){ui.gate=true;ui.gateDim=true;}
  ...
  addCustomer=function(){
    ...
    if(before===6&&ui.batchReady&&!ui.batchRunning){
      captureUndo();
      runBatch789();
      return;
    }
  ...
  function pulseFastStage(){
    ...
    tile.classList.remove('v9-fast-stage');void tile.offsetWidth;tile.classList.add('v9-fast-stage');
    later(()=>tile.classList.remove('v9-fast-stage'),1400);
  }
  ...
  resetExplorer=function(){
    clearOwnTimers();undoStack.length=0;document.querySelector('.v9-coach')?.remove();
    Object.assign(ui,{
      ...
      customer6CoachShown:false,batchReady:false,batchRunning:false,secondHabCoachScheduled:false,secondHabRevealStarted:false,
