(function(){
  if(document.documentElement.dataset.coachingPreviewV5==='1')return;
  document.documentElement.dataset.coachingPreviewV5='1';
  ...
  function showCustomer3Coach(){
    document.querySelector('.v5-customer3-coach')?.remove();
    window.CoachingPreviewV4?.setGate(true);
    const el=document.createElement('div');
    el.className='v4-coach v5-customer3-coach';
  ...
  window.addEventListener('resize',()=>document.querySelectorAll('.v5-customer3-coach').forEach(positionCoach));

  /* Add the agreed coaching cue after customer 3, without adding another action pulse. */
  ...
  /* Defensive guard: never allow a stale action pulse to land on Add 3rd customer. */
  const pulseGuard=new MutationObserver(()=>{
    if(totalCount()!==2)return;
    const btn=addButton();
    if(!btn)return;
    btn.classList.remove('v4-action-pulse','coach-action-pulse');
  });
  pulseGuard.observe(document.body,{subtree:true,attributes:true,attributeFilter:['class']});
  ...
  /* Same proven three-stage Fast Start choreography as v4, with a shorter reward pause. */
